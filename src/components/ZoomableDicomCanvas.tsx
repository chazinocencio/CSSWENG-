...
export default function ZoomableDicomCanvas({
	...
	// Reset zoom when image changes (e.g. series change)
	useEffect(() => {
		resetView();
	}, [image]);
	...
	const animatedStyle = useAnimatedStyle(() => ({
		...
		// Align transform origin with the top-left focal point math
		transformOrigin: '0% 0%',
	}));
	...
	return (
		<View className="items-center">
			<GestureDetector gesture={composed}>
				<View style={{ width: displayWidth, height: displayHeight, overflow: 'hidden' }} className="bg-black rounded-lg">
					<Animated.View
						collapsable={false}
						style={[
			...
			<View className="h-12 justify-center">
				<Pressable
					className="flex-row items-center gap-x-2 rounded-full bg-gray-800/80 px-4 py-2 active:opacity-70"
					onPress={resetView}
				>
					<RotateCcw color="white" size={16} />
					<Text className="text-xs font-semibold text-white">Reset Zoom</Text>
				</Pressable>
			</View>
		</View>
	);
