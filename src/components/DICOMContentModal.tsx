...
interface DICOMContentModalProps {
	...
	ModalProceed: () => void;
	ModalShown: () => void;
	SeriesSelected?: (series: string) => void;
	SelectedSeries?: string;
}

export default function DICOMContentModal({
	Visibility, Content, ZIPContent, TargetFile, ModalProceed, ModalShown, SeriesSelected, SelectedSeries
				...
				<SectionList
					...
					renderSectionHeader={({ section: { title } }) => (
						<View className={`flex-row gap-x-2 items-center rounded-lg p-1 ${SelectedSeries === title ? 'bg-blue-100' : ''}`}>
							...
							<TouchableOpacity onPress={() => SeriesSelected?.(title)} className="flex-1">
	...
	return (
		<Modal animationType="slide" transparent={true} visible={Visibility} onRequestClose={ModalProceed} onShow={ModalShown}>
				...
				>
					...
					<View className="flex-row justify-between items-start pt-6 pb-4 px-6 border-b border-gray-100 bg-gray-50/50">
						<View className="flex-1 mr-4">
							<Text className="text-lg font-bold text-gray-800 leading-6" numberOfLines={2}>
								{decodeURIComponent(TargetFile).substring(decodeURIComponent(TargetFile).lastIndexOf('/') + 1)}
							</Text>
							<Text className="text-xs text-gray-400 mt-1 uppercase tracking-tight">Current Selection</Text>
						</View>

						<TouchableOpacity
							onPress={ModalProceed}
							activeOpacity={0.7}
					...
					<View className="px-6 pt-5 flex-1">
						<View className="flex-row items-center mb-4">
							<View className="w-1 h-full bg-orange-400 rounded-full mr-3" />
							{ ZIPContent ? (
								<View className="flex-col">
									<Text className="text-lg font-bold text-gray-700">Files read can be seen below.</Text>
									<Text className="text-base leading-snug text-gray-700">You may verify the contents before proceeding to the viewer.</Text>
								</View>
							) : (
								<Text className="text-xl font-bold text-gray-700">Metadata</Text>
							)}
						</View>
