...
import { createVolumeJSI, VolumeJSI } from "../../modules/native-dicom";
...
export default function DICOMViewer({
  ...
  const [seriesPlaybackEnabled, setSeriesPlaybackEnabled] =
  ...
  // More Contrast stuff but for 1:x x:1
  const MIN_CURVE = 0; // All White
  const MAX_CURVE = 10; // All Black
  ...
  const getSkiaImage = (metadata: any, frame_pixels: Uint8Array) => {
    try {
      ...
      const pixelCount = metadata.width * metadata.height;
        ...
        // computation on the fly is too expensive which is y phones take a while to render contrast
        const lut = new Uint8Array(256);
        const isLinear = Math.abs(contrastRatio - 1.0) < 0.001;
        for (let j = 0; j < 256; j++) {
          const normalized = j / 255;
          const mapped = isLinear
            ? normalized
            : Math.pow(normalized, contrastRatio);
          lut[j] = Math.floor(mapped * 255);
        }
      ...
      if (frame_pixels.length === pixelCount * 2) {
        ...
        const intercept =
          metadata.rescaleIntercept ?? metadata.rescale_intercept ?? 0;
        ...
        for (let i = 0; i < pixelCount; i++) {
          const pixel = temp[i];
          const linearVal = Math.max(
            0,
            Math.min(255, ((pixel - low) / range) * 255),
          );
          canvas[i] = lut[Math.floor(linearVal)];
        }

        buffer = canvas;
      }

        else if (frame_pixels.length == pixelCount) {
            if (!isLinear) {
                const canvas = new Uint8Array(pixelCount);
                for (let i = 0; i < pixelCount; i++) {
                            canvas[i] = lut[frame_pixels[i]];
                          }
                          buffer = canvas;
                }
            }
  ...
  const LoadSeries = (
    ...
    try {
      ...
      const orthoslice = instance.getOrthoSlice(
        mode,
        index,
        windowWidth,
        windowCenter,
      ...
      if (
        vmd.windowWidth &&
        vmd.windowCenter &&
        currentVolumeRef.current?.uri !== uri
      ) {
  ...
  const handleScoutTouch = (evt: any) => {
    ...
    const scoutUIHeight =
      scoutWidth / (scoutImage.width() / scoutImage.height());
  ...
  useEffect(() => {
    if (!ZIPContent && Content) {
      ...
      const startingSeries =
        initialSeries || Object.keys(ZIPContent.folders)[0];
  ...
  }, [windowWidth, windowCenter, contrastRatio]);
  ...
  const RenderSkia = () => {
    ...
    return (
      <Text className="text-gray-500 mt-10">No image data to render.</Text>
    );
  };

  const scoutWidth = 140;
  const scoutUIHeight = scoutImage
    ? scoutWidth / (scoutImage.width() / scoutImage.height())
    : 140;
    ...
    <View className="flex-1 bg-black relative">
      ...
      {sidebarOpen && (
        ...
        >
          ...
          <View className="mb-6">
            <Text className="text-gray-400 mb-2 text-xs uppercase font-bold">
              Contrast & Brightness
            </Text>
            ...
            <View className="mb-4">
              ...
              <View className="flex-row items-center">
                <TouchableOpacity
                  onPress={() =>
                    setWindowWidth((prev) => Math.max(MIN_WW, prev - 50))
                  }
                  className="bg-gray-700 p-2 rounded-l-md px-3 border-r border-gray-600"
                >
                ...
                <View
                  ...
                  onLayout={(e) =>
                    setTrackWidth(e.nativeEvent.layout.width - 16)
                  }
                ...
                >
                  <View
                    className="bg-gray-600 h-1 rounded-full relative"
                    pointerEvents="none"
                  >
                    <View
                      style={{
                        left: `${Math.min(100, Math.max(0, ((windowWidth - MIN_WW) / (MAX_WW - MIN_WW)) * 100))}%`,
                      }}
                ...
                <TouchableOpacity
                  onPress={() =>
                    setWindowWidth((prev) => Math.min(MAX_WW, prev + 50))
                  }
                  className="bg-gray-700 p-2 rounded-r-md px-3 border-l border-gray-600"
                >
                  <Text className="text-white font-bold">+</Text>
                </TouchableOpacity>
              </View>
            ...
            <View className="mb-4">
              <View className="flex-row justify-between mb-1">
                <Text className="text-white text-[10px]">
                  LEVEL: {Math.round(windowCenter)}
                </Text>
              </View>
              <View className="flex-row items-center">
                <TouchableOpacity
                  onPress={() =>
                    setWindowCenter((prev) => Math.max(MIN_WL, prev - 20))
                  }
                  className="bg-gray-700 p-2 rounded-l-md px-3 border-r border-gray-600"
                >
                  <Text className="text-white font-bold">-</Text>
                ...
                >
                  ...
                  >
                    <View
                      style={{
                        left: `${Math.min(100, Math.max(0, ((windowCenter - MIN_WL) / (MAX_WL - MIN_WL)) * 100))}%`,
                      }}
                ...
                <TouchableOpacity
                  onPress={() =>
                    setWindowCenter((prev) => Math.min(MAX_WL, prev + 20))
                  }
                  className="bg-gray-700 p-2 rounded-r-md px-3 border-l border-gray-600"
                >
                  <Text className="text-white font-bold">+</Text>
            ...
            <View className="mb-4">
              <View className="flex-row justify-between items-center mb-1">
                <Text className="text-white text-[10px]">
                  RATIO: {contrastRatio.toFixed(2)}x{" "}
                  {Math.abs(contrastRatio - 1.0) < 0.05
                    ? "(Linear 1:1)"
                    : contrastRatio < 1.0
                      ? "(1:x Dark Stretch)"
                      : "(x:1 Bright Stretch)"}
                ...
                <TouchableOpacity
                  onPress={() => setContrastRatio(1.0)}
                  className="bg-gray-700 px-1.5 py-0.5 rounded border border-gray-600"
                >
              ...
              <View className="flex-row items-center">
                <TouchableOpacity
                  onPress={() =>
                    setContrastRatio((prev) => Math.max(MIN_CURVE, prev - 0.1))
                  }
                  className="bg-gray-700 p-2 rounded-l-md px-3 border-r border-gray-600"
                >
                  <Text className="text-white font-bold">-</Text>
                ...
                <View
                  className="flex-1 bg-gray-800 h-9 justify-center px-2"
                  onStartShouldSetResponder={() => true}
                  onResponderGrant={handleCurveScrub}
                  onResponderMove={handleCurveScrub}
                >
                  <View
                    className="bg-gray-600 h-1 rounded-full relative"
                    pointerEvents="none"
                  >
                    <View
                      style={{
                        left: `${((1.0 - MIN_CURVE) / (MAX_CURVE - MIN_CURVE)) * 100}%`,
                      }}
                      className="absolute -top-1 w-0.5 h-3 bg-gray-400"
                    />
                    <View
                      style={{
                        left: `${Math.min(100, Math.max(0, ((contrastRatio - MIN_CURVE) / (MAX_CURVE - MIN_CURVE)) * 100))}%`,
                      }}
                      className="absolute -top-1.5 w-4 h-4 -ml-2 bg-purple-500 rounded-full border-2 border-white"
                    />
                  </View>
                </View>
                ...
                <TouchableOpacity
                  onPress={() =>
                    setContrastRatio((prev) => Math.min(MAX_CURVE, prev + 0.1))
                  }
                  className="bg-gray-700 p-2 rounded-r-md px-3 border-l border-gray-600"
                >
                  <Text className="text-white font-bold">+</Text>
            ...
            <View className="flex-row flex-wrap gap-2">
              ...
              <TouchableOpacity
                ...
                className="bg-purple-600/30 px-2 py-1 rounded-md border border-purple-500/50"
              >
              ...
              <TouchableOpacity
                ...
                className="bg-red-600/30 px-2 py-1 rounded-md border border-red-500/50"
              >
          ...
          {ZIPContent && (
            <View className="mb-4 flex-1">
              <Text className="text-gray-400 mb-2 text-sm uppercase font-bold">
                Series
              </Text>
          ...
          <TouchableOpacity
            onPress={onClose}
            className="mt-auto bg-red-600 p-3 rounded-lg"
          >
      ...
      <View className="flex-1 justify-center items-center relative">
        {patientName && patientSex && (
          ...
          >
            <Text className="text-white text-md font-semibold tracking-wide">
              {patientName}{" "}
              <Text className="text-gray-400">({patientSex})</Text>
            </Text>
          </View>
        )}
        ...
        {ZIPContent && seriesName && (
          <View className="absolute bottom-10 flex-row p-2 bg-gray-800/80 rounded-2xl items-center">
            <Pressable
              className={`justify-center m-2 p-3 rounded-full ${seriesPlaybackEnabled ? "bg-blue-500" : "bg-orange-500"}`}
            ...
            <View className="flex-row gap-x-2 items-center px-4">
              <Pressable
                className="p-1 active:bg-gray-600 rounded-full"
                onPress={() =>
                  LoadSeries(
                    seriesName,
